      ...
      <div className="max-w-[1400px] mx-auto px-8 md:px-14 py-24 md:py-32 space-y-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          ...
          <div className="relative h-[300px] md:h-[420px] overflow-hidden bg-[#060608]">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#060608]/80 via-transparent to-[#060608]/30" />
            <div className="absolute inset-0 bg-gradient-to-l from-transparent to-[#0D0D0D]/40" />
            <div className="absolute inset-0 bg-[#0A0800]/20" />
            {/* Corner accents */}
            {["top-0 left-0 border-t border-l","top-0 right-0 border-t border-r","bottom-0 left-0 border-b border-l","bottom-0 right-0 border-b border-r"].map((c, idx) => (
              <div key={idx} className={`absolute ${c} w-5 h-5`} style={{ borderColor: "rgba(201,168,76,0.2)" }} />
            ))}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 border border-white/[0.06] divide-y sm:divide-y-0 sm:divide-x divide-white/[0.06]">
          {pillars.map((p, i) => (
            ...
            >
              <div className="absolute top-0 left-0 w-full h-[1px] bg-gold scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left" />
              <div className="flex items-baseline justify-between">
                ...
                <span className="font-mono text-gold/50 text-[9px]">{p.num}</span>
              </div>
              <p
        ...
        {/* ── CLOSING QUOTE ── */}
        <div className="border-t border-white/[0.05] pt-14 text-center">
          <p
