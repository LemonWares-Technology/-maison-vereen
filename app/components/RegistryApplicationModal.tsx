...
interface RegistryApplicationModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export default function RegistryApplicationModal({ isOpen, onClose }: RegistryApplicationModalProps) {
  ...
  useEffect(() => {
    ...
    STEPS.forEach(({ id }) => {
      ...
      const obs = new IntersectionObserver(
        ...
        { root: container, rootMargin: "-35% 0px -45% 0px", threshold: 0 }
      );
      obs.observe(el);
      observers.push(obs);
    });
  ...
  return (
    ...
    >
      <div
        ...
        className="relative w-[95%] max-w-6xl max-h-[95vh] my-4 sm:my-10 bg-[#060506] border border-gold/20 shadow-2xl overflow-y-auto"
      >
        <button
          onClick={onClose}
          className="fixed sm:absolute top-5 right-5 z-20 text-gold hover:text-[#EDE8DE] transition-colors duration-300 p-1"
          aria-label="Close"
        >
        ...
        <div className="md:grid md:grid-cols-2 md:items-start">
          ...
          <aside className="relative bg-[#060506] border-b md:border-b-0 md:border-r border-gold/20">
            <div className="w-[90%] max-w-xl mx-auto md:mx-0 md:ml-[8%] py-10 md:py-14 space-y-10 md:space-y-12 pb-14">
              {/* Intro */}
          ...
          <div id="registry-application" className="relative bg-[#060506] scroll-mt-4">
            <div className="w-[95%] max-w-2xl mx-auto py-10 md:py-14 md:px-8 space-y-10">
              ...
              {submitted ? (
                <div className="border border-gold/40 p-8 space-y-4 text-center">
                  ...
                  <button
                    onClick={onClose}
                    className="border border-gold/60 hover:border-gold text-gold px-8 py-3 text-xs tracking-[0.28em] uppercase font-medium transition-colors"
                  >
                    Return to the House
                  </button>
                </div>
